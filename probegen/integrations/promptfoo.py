...
from probegen.models.eval_case import flatten_expected_output
...
class PromptfooWriter:
    def write_tests(
        ...
        *,
        ...
        payload["description"] = (
            f"Probegen probes for {artifact_path or 'artifacts'}"
            + (f" (PR #{pr_number})" if pr_number is not None else "")
        )
        payload["tests"] = tests
        header = f"# Generated by probegen v{version} — commit {commit_sha}\n"
